    ...
    <div className="space-y-6">
      ...
      {mcData && (
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-xl bg-[#F4EFE6] border border-[#E7E0D3] text-xs space-y-2 text-[#1C1917]">
            <span className="font-bold text-[#1C1917] text-sm flex items-center gap-1.5 font-serif">
              <ShieldCheck className="w-4 h-4 text-[#C85A32]" /> What is Monte Carlo Risk Analysis in Plain English?
            </span>
            ...
            <div className="grid md:grid-cols-3 gap-3 pt-2">
              <div className="p-2.5 rounded bg-white border border-[#E7E0D3]">
                <span className="font-semibold text-emerald-700 block mb-0.5">Probability of Profit ({mcData.metrics.probabilities.probability_of_profit}%)</span>
                <p className="text-[#78716C]">Out of 1,000 simulated months, your business made a profit in {Math.round(mcData.metrics.probabilities.probability_of_profit * 10)} out of 1,000 runs.</p>
              </div>
              <div className="p-2.5 rounded bg-white border border-[#E7E0D3]">
                <span className="font-semibold text-[#C85A32] block mb-0.5">Expected Mean Profit (₦{mcData.metrics.mean.toLocaleString()})</span>
                <p className="text-[#78716C]">Your average expected monthly net profit across all simulated market scenarios.</p>
              </div>
              <div className="p-2.5 rounded bg-white border border-[#E7E0D3]">
                <span className="font-semibold text-rose-700 block mb-0.5">P5 Worst Case (₦{mcData.metrics.percentiles.p5.toLocaleString()})</span>
                <p className="text-[#78716C]">Even in a tough month (bottom 5% worst cases), your estimated profit stays around this amount.</p>
              </div>
            </div>
          </div>
          ...
          <div className="editorial-card p-6 space-y-4">
            <h3 className="font-bold text-[#1C1917] font-serif text-base">Confidence & Percentile Markers</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 text-center text-xs">
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P5 (Worst 5%)</span>
                <span className="font-mono font-bold text-rose-700">₦{mcData.metrics.percentiles.p5.toLocaleString()}</span>
              </div>
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P10</span>
                <span className="font-mono font-bold text-amber-700">₦{mcData.metrics.percentiles.p10.toLocaleString()}</span>
              </div>
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P25</span>
                <span className="font-mono font-bold text-[#1C1917]">₦{mcData.metrics.percentiles.p25.toLocaleString()}</span>
              </div>
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P50 (Average Month)</span>
                <span className="font-mono font-bold text-[#1C1917]">₦{mcData.metrics.percentiles.p50.toLocaleString()}</span>
              </div>
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P75</span>
                <span className="font-mono font-bold text-[#1C1917]">₦{mcData.metrics.percentiles.p75.toLocaleString()}</span>
              </div>
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P90</span>
                <span className="font-mono font-bold text-emerald-700">₦{mcData.metrics.percentiles.p90.toLocaleString()}</span>
              </div>
              <div className="p-3 rounded-lg bg-white border border-[#E7E0D3]">
                <span className="text-[#78716C] block mb-1">P95 (Best 5%)</span>
                <span className="font-mono font-bold text-emerald-800">₦{mcData.metrics.percentiles.p95.toLocaleString()}</span>
              </div>
            </div>
          </div>
        </div>
